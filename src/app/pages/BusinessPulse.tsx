...
import {
  ...
  XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, Cell, Area, AreaChart, LabelList, ReferenceLine
} from "recharts";
...
const revenueData = [
  { month: 'Jan', firm: 17,   prospId: 0,    prospUnid: 0,   revenue: 17,   type: 'actual',   cumulativeTarget: 18,  cumulativeActual: 17,    cumulativeForecast: null  },
  { month: 'Feb', firm: 18,   prospId: 0,    prospUnid: 0,   revenue: 18,   type: 'actual',   cumulativeTarget: 36,  cumulativeActual: 35,    cumulativeForecast: null  },
  { month: 'Mar', firm: 18.4, prospId: 0,    prospUnid: 0,   revenue: 18.4, type: 'actual',   cumulativeTarget: 54,  cumulativeActual: 53.4,  cumulativeForecast: 53.4  },
  { month: 'Apr', firm: 15,   prospId: 5,    prospUnid: 3,   revenue: 23,   type: 'forecast', cumulativeTarget: 76,  cumulativeActual: null,  cumulativeForecast: 76.4  },
  { month: 'May', firm: 14,   prospId: 6,    prospUnid: 5,   revenue: 25,   type: 'forecast', cumulativeTarget: 100, cumulativeActual: null,  cumulativeForecast: 101.4 },
  { month: 'Jun', firm: 13,   prospId: 7,    prospUnid: 7,   revenue: 27,   type: 'forecast', cumulativeTarget: 126, cumulativeActual: null,  cumulativeForecast: 128.4 },
  { month: 'Jul', firm: 10,   prospId: 6,    prospUnid: 6,   revenue: 22,   type: 'forecast', cumulativeTarget: 150, cumulativeActual: null,  cumulativeForecast: 150.4 },
  { month: 'Aug', firm: 8,    prospId: 6,    prospUnid: 6,   revenue: 20,   type: 'forecast', cumulativeTarget: 172, cumulativeActual: null,  cumulativeForecast: 170.4 },
  { month: 'Sep', firm: 10,   prospId: 8,    prospUnid: 8,   revenue: 26,   type: 'forecast', cumulativeTarget: 198, cumulativeActual: null,  cumulativeForecast: 196.4 },
  { month: 'Oct', firm: 10,   prospId: 9,    prospUnid: 9,   revenue: 28,   type: 'forecast', cumulativeTarget: 226, cumulativeActual: null,  cumulativeForecast: 224.4 },
  { month: 'Nov', firm: 10,   prospId: 10,   prospUnid: 10,  revenue: 30,   type: 'forecast', cumulativeTarget: 256, cumulativeActual: null,  cumulativeForecast: 254.4 },
  { month: 'Dec', firm: 10,   prospId: 11,   prospUnid: 11,  revenue: 32,   type: 'forecast', cumulativeTarget: 288, cumulativeActual: null,  cumulativeForecast: 286.4 },
];
...
const Spark = ({ data, color = "#3b82f6", height = 22 }: { data: number[]; color?: string; height?: number }) => {
  ...
  return (
    <svg width={w} height={totalH} viewBox={`0 0 ${w} ${totalH}`} className="overflow-visible shrink-0">
      <defs>
        ...
        return (
          <React.Fragment key={i}>
            ...
            {(i === 0 || i === data.length - 1 || i === Math.floor(data.length / 2)) && (
              <text x={x} y={h + labelH + 1} textAnchor="middle" fontSize="6" fill="#94a3b8" fontWeight="600">
                M{i + 1}
              </text>
            )}
          </React.Fragment>
...
interface PulseCardProps {
  ...
  insight?: string;
  tooltipDetails?: { label: string; value: string; rag?: "red" | "amber" | "green" }[];
}
    ...
    <SmartTooltip content={
      <div className="space-y-1.5">
        ...
        {tooltipDetails?.map((d, i) => {
          const dRag = d.rag ? ragConfig[d.rag] : null;
          return (
            <div key={i} className="flex justify-between gap-4 text-[10px]">
              <span className="text-slate-500">{d.label}</span>
              <span className={dRag ? `${dRag.text}` : "text-slate-800"} style={{ fontWeight: 600 }}>{d.value}</span>
            </div>
          );
        })}
      ...
      {payload.map((p: any, i: number) => {
        ...
        return (
          <div key={i} className="flex items-center gap-2 mb-0.5">
            ...
            <span className="text-slate-800" style={{ fontWeight: 700 }}>{p.value}M SAR</span>
          </div>
        );
      })}
      {cumTarget && cumValue && (
        <div className={`mt-1 pt-1 border-t border-slate-200/60 text-[9px] ${rag.text}`} style={{ fontWeight: 600 }}>
          {cumValue >= cumTarget
            ? `On track: +${(cumValue - cumTarget).toFixed(1)}M SAR above target`
            : `Gap: -${(cumTarget - cumValue).toFixed(1)}M SAR behind target (${(((cumValue - cumTarget) / cumTarget) * 100).toFixed(1)}%)`}
        </div>
      )}
    </div>
  );
    ...
    <div className="flex flex-col gap-2 max-w-[1600px] mx-auto px-4 py-2.5">
      ...
      <div className="grid grid-cols-4 gap-2">
        <PulseCard
          ...
          value="SAR 53.4M" label="YTD Revenue" target="SAR 54M"
          ...
          insight="Revenue gap widening \u2014 Q2 pipeline acceleration needed to close SAR 17.6M H1 shortfall."
          tooltipDetails={[
            { label: "YTD Actual", value: "SAR 53.4M" },
            { label: "YTD Target", value: "SAR 54.0M" },
            { label: "Variance", value: "-SAR 0.6M (-1.1%)", rag: "amber" },
            { label: "Gross Margin", value: "26.8% (+2.1% vs Forecast)", rag: "green" },
            { label: "Cash Collection", value: "SAR 48.2M (90.3% of revenue)", rag: "green" },
          ]}
        />
        <PulseCard
          ...
          tooltipDetails={[
            ...
            { label: "On Schedule", value: "36 (77%)", rag: "amber" },
            { label: "Delayed", value: "6 (12%)", rag: "red" },
            { label: "CPI", value: "0.94", rag: "amber" },
            { label: "At-Risk Revenue", value: "SAR 8.2M", rag: "red" },
          ]}
        />
        <PulseCard
          ...
          insight="Bench headcount growing. 142 undeployed staff costing ~SAR 1.8M/month in idle capacity."
          tooltipDetails={[
            { label: "Total Headcount", value: "1,247" },
            { label: "Billable", value: "1,022 (82%)", rag: "amber" },
            { label: "Bench", value: "142", rag: "amber" },
            { label: "DSO", value: "140 days (+12 vs target)", rag: "red" },
            { label: "Idle Cost/Month", value: "~SAR 1.8M", rag: "red" },
            { label: "Deployment Rate", value: "78% (503 of 645)", rag: "amber" },
          ]}
        />
        <PulseCard
          ...
          tooltipDetails={[
            { label: "CSAT Score", value: "88% (+1% MoM)", rag: "green" },
            { label: "NPS", value: "42 (+5 QoQ)", rag: "green" },
            { label: "Active Escalations", value: "2 (down from 3)", rag: "green" },
            { label: "Repeat Business Rate", value: "73%", rag: "green" },
      ...
      <div className="grid grid-cols-4 gap-2">
        <PulseCard
          ...
          icon={PieChart} status="green"
          value="SAR 58M" label="Weighted Pipeline" target="SAR 50M"
          mom="5.5%" momPositive={true}
          yoy="12.3%" yoyPositive={true}
          sparkData={momData.pipeline} sparkColor="#0ea5e9"
          metric2Label="Win Rate" metric2Value="34%" metric2Variance="+3%" metric2Positive={true}
          insight="Pipeline exceeds target by 16%. 12 proposals submitted this month \u2014 4 deals in final negotiation stage."
          tooltipDetails={[
            { label: "This Month Won", value: "SAR 6.2M (3 deals)", rag: "green" },
            { label: "YTD Won", value: "SAR 38.4M (18 deals)", rag: "green" },
            { label: "Proposals Submitted (Month)", value: "12 (SAR 14.8M)" },
            { label: "Proposals Submitted (YTD)", value: "47 (SAR 62.1M)" },
            { label: "Weighted Pipeline", value: "SAR 58M vs SAR 50M Target", rag: "green" },
            { label: "Win Rate", value: "34% (+3% MoM)", rag: "green" },
            { label: "Avg Deal Size", value: "SAR 2.1M (+8% YoY)" },
            { label: "Sales Cycle", value: "68 days (-4d MoM)", rag: "green" },
            { label: "Conversion (Proposal\u2192Won)", value: "38%", rag: "amber" },
          ]}
        />
        <PulseCard
          ...
          tooltipDetails={[
            { label: "Current Rate", value: "27.0%", rag: "amber" },
            { label: "Green Threshold", value: "30.0%" },
            { label: "Gap", value: "-3.0% (42 hires needed)", rag: "red" },
            { label: "Pipeline", value: "28 candidates in process", rag: "amber" },
          ]}
        />
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="col-span-2 bg-white rounded-lg border border-slate-200 shadow-sm flex flex-col">
          <div className="px-3 py-1.5 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2 text-[8px] text-slate-400" style={{ fontWeight: 600 }}>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-blue-700 inline-block" /> Firm</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-sky-400 inline-block" /> Prosp. Identified</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-sky-200 inline-block" /> Prosp. Unidentified</span>
              <span className="flex items-center gap-1"><span className="w-1 h-3 bg-red-500 inline-block" /> Cut-off</span>
              <span className="flex items-center gap-1"><span className="w-3 border-t-[1.5px] border-dashed border-amber-500 inline-block" /> Cum. Target</span>
              <span className="flex items-center gap-1"><span className="w-3 border-t-[1.5px] border-blue-800 inline-block" /> Cum. Actual</span>
              <span className="flex items-center gap-1"><span className="w-3 border-t-[1.5px] border-dashed border-sky-400 inline-block" /> Cum. Forecast</span>
            </div>
          </div>
          <div className="px-1 pb-1 flex-1 min-w-0" style={{ height: 180 }}>
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={revenueData} margin={{ top: 14, right: 45, left: -24, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: "#94a3b8" }} />
                <YAxis yAxisId="left" axisLine={false} tickLine={false} tick={{ fontSize: 8, fill: "#94a3b8" }} tickFormatter={(v) => `${v}M`} domain={[0, 40]} width={42} />
                <YAxis yAxisId="right" orientation="right" axisLine={false} tickLine={false} tick={{ fontSize: 8, fill: "#94a3b8" }} tickFormatter={(v) => `${v}M`} domain={[0, 320]} width={42} />
                <RechartsTooltip content={<ChartTooltip />} />
                <Bar yAxisId="left" dataKey="firm" name="Firm Projects" stackId="revenue" radius={[0, 0, 0, 0]} barSize={18} fill="#1d4ed8" />
                <Bar yAxisId="left" dataKey="prospId" name="Prosp. Identified" stackId="revenue" radius={[0, 0, 0, 0]} barSize={18} fill="#38bdf8" />
                <Bar yAxisId="left" dataKey="prospUnid" name="Prosp. Unidentified" stackId="revenue" radius={[3, 3, 0, 0]} barSize={18} fill="#bae6fd" />
                <ReferenceLine yAxisId="left" x="Mar" stroke="#ef4444" strokeWidth={2} strokeDasharray="4 2" label={{ value: "Cut-off", position: "top", fontSize: 7, fill: "#ef4444", fontWeight: 700 }} />
                <Line yAxisId="right" type="monotone" dataKey="cumulativeTarget" name="Cum. Target" stroke="#f59e0b" strokeWidth={1.5} strokeDasharray="6 4" dot={{ r: 2, fill: "#f59e0b", strokeWidth: 0 }}>
                  <LabelList dataKey="cumulativeTarget" content={({ x, y, value, index }: any) => { if (index % 3 !== 0 && index !== 11) return <text key={`ct-${index}`} />; return <text key={`ct-${index}`} x={x} y={(y || 0) - 5} textAnchor="middle" fontSize={7} fill="#f59e0b" fontWeight={600}>{value}</text>; }} />
                </Line>
                <Line yAxisId="right" type="monotone" dataKey="cumulativeActual" name="Cum. Actual" stroke="#1e3a8a" strokeWidth={2} dot={{ r: 2.5, fill: "#1e3a8a", strokeWidth: 0 }} connectNulls={false}>
                  <LabelList dataKey="cumulativeActual" content={({ x, y, value, index }: any) => { if (value === null || value === undefined) return <text key={`ca-${index}`} />; return <text key={`ca-${index}`} x={x} y={(y || 0) + 12} textAnchor="middle" fontSize={7} fill="#1e3a8a" fontWeight={700}>{value}</text>; }} />
                </Line>
                <Line yAxisId="right" type="monotone" dataKey="cumulativeForecast" name="Cum. Forecast" stroke="#38bdf8" strokeWidth={1.5} strokeDasharray="4 3" dot={{ r: 2, fill: "#38bdf8", strokeWidth: 0 }} connectNulls={false}>
                  <LabelList dataKey="cumulativeForecast" content={({ x, y, value, index }: any) => { if (value === null || value === undefined) return <text key={`cf-${index}`} />; if (index % 3 !== 0 && index !== 11) return <text key={`cf-${index}`} />; return <text key={`cf-${index}`} x={x} y={(y || 0) - 5} textAnchor="middle" fontSize={7} fill="#38bdf8" fontWeight={600}>{value}</text>; }} />
                </Line>
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="bg-white rounded-lg border border-red-200/40 shadow-sm flex flex-col">
          ...
          <div className="px-2.5 flex-1 flex flex-col gap-1.5">
            <RiskItem title="Resource Bottlenecks Q2" exposure="SAR 2.4M" severity="Critical" impact="Delays on 3 mega-projects" />
            <RiskItem title="Client Approval Delays" exposure="SAR 1.2M" severity="Major" impact="4 projects pending sign-off" />
            <RiskItem title="Margin Leakage \u2014 Fixed Price" exposure="SAR 0.8M" severity="Major" impact="Scope creep on 2 contracts" />
